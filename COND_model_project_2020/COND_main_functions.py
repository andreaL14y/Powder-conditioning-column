from COND_support_functions import*
...
def conditioning_column(moisture_matrix, t, space_step, n_space_steps, n_height_steps):
    ...
    ##################################### CREATE T_G MATRIX ############################################################
    glass_transition_temp_vector = compute_glass_temp_mix(1 - moisture_particle_am_vector, glass_temp_lactose, glass_temp_water)
    ...
    change_amorphous_material = np.zeros([n_height_steps, n_space_steps])
    ##################################### UPDATE MOISTURE ##############################################################
    ...
    heat_of_sorption_gas    = density_particle * porosity_powder * constant * moisture_vapor_heat_capacity * \
