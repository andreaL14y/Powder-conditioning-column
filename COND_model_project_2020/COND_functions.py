...
def compute_laplacian(array, boundary, step_length, verbose=False, single=False):
    #     m_sur_bounds = np.array([m_gas_sur, m_void[0]])
    n_steps = array.size

    if single:
        matrix = np.insert(array, 0, boundary[0])  # Add boundary conditions to array at 0
        matrix = np.append(matrix, boundary[1])  # Add final conditions to array again at end
