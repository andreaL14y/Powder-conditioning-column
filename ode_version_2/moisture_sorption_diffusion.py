...
def compute_system(initials, t, n_space_steps, step_length, verbose=False):
    ...
    laplacian_conc = compute_laplacian(m_void, m_gas_sur, step_length) * (density_gas * porosity_powder)    #TODO: no porosity, density in laplacian
    ...
    gas_density_curr = compute_density_air(temp_celsius_prev, m_void_prev)    #TODO: this insted of gas_density
    laplacian_energy = compute_laplacian(enthalpy_vapor * m_void_prev * gas_density_curr, enthalpy_vapor_initial * m_gas_sur * gas_density_sur, step_length, False)

    change_energy_diffusion = diffusivity_eff * laplacian_energy

    temp_laplacian = compute_laplacian(temp_celsius_prev, temp_initial_celsius, step_length)
    ...
    temp_diff = temp_kelvin_prev - glass_temp
    change_amorph = compute_kinetics_avrami(amount_am, temp_diff)

    change_energy_crystallization = change_amorph * heat_of_crystallization
    change_energy = change_energy_diffusion + change_energy_conduction + change_energy_crystallization

    if verbose and counter % divider == 0:
        print('\n')
        print('         TIME', int(t), 's;', int(t / 60), 'min')
        np.set_printoptions(formatter={'float': '{: .1e}'.format})
        ...
        print('m_void:'.ljust(tabs), m_void)
        print('m_gas_sur:'.ljust(tabs), f'{m_gas_sur:.1f}')
        print('Water act:'.ljust(tabs), water_activity)
    ...
    system_change = np.concatenate([m_diffusion, change_amorph, change_energy]).astype('float64')
    m_void_prev = m_void
...
def plot_tam():
    ...
    axs[0, 1].plot(time, t_tg_diff_avg,         label='T-Tg', color=temp_color)
    ...
    axs[1, 1].plot(time, amorphicity_avg, label='Amorphicity', color=am_color)
    ...
    axs[1, 1].plot(time, amorphicity_all[:, 2 * int(n_space_steps/3)],  label=f'{2*int(n_space_steps/3)}', color='limegreen', linestyle='--')
    ...
    axs[2, 1].plot(time, energy_avg, label='Average energy', color='darkorange')
    ...
    axs[2, 2].plot(time, temp_all_celsius[:, 0],                label='0', color='goldenrod', linestyle='--')
    ...
    plt.savefig('system_w_derivative2.pdf')
    plt.show()
...
m_tot_con_vector = np.zeros(n_space_steps) + m_particle_tot_conc_initial + m_void_conc_initial
...
initials = np.concatenate([m_tot_con_vector, initial_amorphicity_vector, energy_initial_vector])      # kg/m3, kg/kg
...
for t in range(1, 1+resolution):
    ...
    m_void_all[t, :] = compute_H_from_aw_temp(water_activity_all[t, :], temp_all_celsius[t-1, :])
...
m_void_conc_all         = m_void_all * porosity_powder * density_gas
...
m_void_avg               = m_void_conc_avg / (porosity_powder * density_gas)
amorphicity_avg         /= n_space_steps
total_water_avg         /= n_space_steps
energy_avg              /= n_space_steps

water_activity_avg      = compute_water_activity_from_m_void(m_void_avg, p_saturated)
