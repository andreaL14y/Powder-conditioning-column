...
def compute_system(initials, t, n_space_steps, step_length, verbose=False, time_continue=False):
    ...
    optimized_aw = scipy.optimize.minimize(compute_H_and_M_iteratively, water_activity_just_used, method='SLSQP',
                                           ...
                                           # bounds=((0, 1),) * n_space_steps, options={'ftol': 1e-08})   #TODO: decrease!
    ...
    change_amorph = compute_kinetics_avrami(amount_am/amorphous_material_initial, temp_diff)    # TODO: added
    ...
    change_energy_crystallization *= amorphous_material_initial            # TODO: added
    ...
    m_diffs_p_conc = m_diffs_p * (1 - porosity_powder) * density_particle   # positive during sorption, then neg
    change_energy_binding = m_diffs_p_conc * heat_binding_diffs / density_water   # positive during sorption, then neg
    ...
    system_change       = np.concatenate([change_moisture, change_amorph, change_energy,
                                          water_activity_change, m_sur_change, temp_change]).astype('float64')
...
def plot_tam():
    ...
    axs[1, 2].plot(time, heat_of_cryst_vector, label='Crystallization', color='darkgoldenrod')
    ...
    plt.savefig(f'data_and_plots/exp {exp}, time {hours}, n {n_space_steps} sim.pdf')
    plt.show()
...
#
# np.save(f'data_and_plots/exp {exp}, time {hours}, n {n_space_steps}', computed_system)
#
