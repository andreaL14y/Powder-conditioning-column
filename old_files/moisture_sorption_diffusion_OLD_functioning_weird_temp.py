...
def compute_system(initials, t, n_space_steps, step_length, verbose=False, time_continue=False):
    ...
    # temp_celsius_sur_prev = initials[n_space_steps * 5:n_space_steps * 6]         #TODO: uncomment
    ...
    # m_void                  = compute_H_from_water_activity_temp(water_activity, temp_celsius_prev)
    ...
    # temp_kelvin, temp_celsius = compute_temp_from_energy(m_void_prev, m_particle_tot_prev, total_energy, excess_water, verbose=False)
    ...
    # gas_density_curr          = compute_density_air(m_void)
    energy_vapor              =  m_void * compute_enthalpy_vapor(temp_celsius) * gas_density

    ######################################### AIR IN AMPOULE ###########################################################
    # gas_density_sur = compute_density_air(temp_celsius_sur_prev, m_sur)
    gas_density_sur = compute_density_air(m_sur)

                        ###################
    # temp_celsius_sur = (energy_sur_air - m_sur * heat_of_evaporation_water) / (heat_capacity_air + m_sur * heat_capacity_vapor)
    # m_vaporized_sur = m_gas_sur - m_sur[0]
    # enthalpy_vap_sur = m_vaporized_sur * (heat_capacity_vapor * temp_celsius_sur_prev[0] + heat_of_evaporation_water)    # positive during absorption, phase 1. J/kg
    # enthalpy_vap_sur = m_vaporized_sur * heat_of_evaporation_water                # positive during absorption, phase 1. J/kg
    # temp_change_from_evap = enthalpy_vap_sur / heat_capacity_vapor              # J/kg / J/(kg K) = K
                        ###################

    m_sur_bounds = np.array([m_gas_sur, m_void[0]])
    # Add the newly vaporized amount to already existing amount
    # m_sur_bounds = np.array([m_vaporized_sur + m_sur[0], m_void[0]])
    ...
    # m_from_sur_change = m_from_sur * moisture_diffusivity                           # kg/m5 * m2/s = kg/(m3 s)
    # enthalpy_vap_sur = m_from_sur_change * heat_of_evaporation_water                # positive during absorption, phase 1. J/kg

    # temp_sur_bounds = np.array([temp_initial_celsius, temp_celsius[0]])
    # # temp_sur_bounds = np.array([temp_celsius_sur_prev[0] - temp_change_from_evap, temp_celsius[0]])
    # temp_sur_laplacian = compute_laplacian(temp_celsius_sur_prev, temp_sur_bounds, step_length_air, double_bc=True) #TODO: uncomment this and below
    # temp_sur_change = temp_sur_laplacian * conductivity_gas / (heat_capacity_air * gas_density_sur)
    # temp_sur_change[0] -= temp_change_from_evap
    # energy_vapor_sur = m_sur * compute_enthalpy_vapor(temp_celsius_sur) * gas_density_sur        #TODO: same as below?? Used in line 92

    # Energy change air ampoule, TODO: comment
    # temp_sur_bounds = np.array([temp_initial_celsius, temp_celsius[0]])
    # temp_sur_laplacian = compute_laplacian(temp_celsius_sur, temp_sur_bounds, step_length_air, double_bc=True)
    # print(temp_sur_laplacian)

    # change_energy_sur_cond = temp_sur_laplacian * conductivity_gas       # J/(m3 s)

    # e_sur_bounds = np.array([energy_vapor_sur[0], energy_vapor[0]])  # TODO: comment
    # energy_vapor_sur_laplacian = compute_laplacian(energy_sur_air, e_sur_bounds, step_length_air, double_bc=True)
    # change_energy_sur_diffusion = moisture_diffusivity * energy_vapor_sur_laplacian
    # change_energy_sur = change_energy_sur_diffusion + change_energy_sur_cond      # m2/s * J/m5 (?)
    # change_energy_sur[0] -= enthalpy_vap_sur
    #TODO: end comment
    # energy_vapor_sur = m_sur * compute_enthalpy_vapor(temp_celsius_sur_prev) * gas_density_sur        #TODO: same as below?? Used in line 92
    ...
    # energy_diff_laplacian = compute_laplacian(energy_vapor, energy_vapor_sur[-1], step_length)
    # energy_diff_laplacian = compute_laplacian(energy_vapor, energy_sur_vapor, step_length)
    ...
    # temp_bounds = np.array([temp_celsius_sur[-1], temp_initial_celsius])
    ...
    change_energy_crystallization = - change_energy_crystallization         # Positive
                    ###
    # m_freed = (m_particle_am - m_particle_cryst) * change_amorph                    # kg/kg solid lactose, neg
    # m_freed *= (1-porosity_powder) * density_particle                               # kg/m3 water
    # h_diffs = m_void_prev
    # # change_energy_evap_sur       = m_freed * (heat_capacity_vapor * temp_celsius + heat_of_evaporation_water)              # Negative, kg/(m3 s) * J/kg = J/(m3 s)
    #
    # # excess_water_conc = excess_water/(porosity_powder * gas_density_curr)
    # excess_water_conc = -m_freed * (1-porosity_powder) * density_particle/(porosity_powder * gas_density_curr)
    # change_energy_evap_sur =  excess_water_conc * (heat_capacity_vapor * temp_celsius + heat_of_evaporation_water) \
    #                              - m_freed * heat_capacity_water * temp_celsius                         # Positive, kg/(m3 s) * J/kg = J/(m3 s)
    # change_energy_evap_sur = np.where(change_energy_evap_sur < 50, 0, change_energy_evap_sur)
                    ###
    ...
    # change_energy_binding = np.where(change_amorph < 0, 0, change_energy_binding)
    ...
    # change_energy_evap_sur = change_energy_diffusion_sur
    # change_energy[0] = change_energy[0] - change_energy_evap_sur
    ...
    system_change       = np.concatenate([change_moisture, change_amorph, change_energy,
                                          water_activity_change, m_sur_change,   #temp_sur_change, change_energy_sur
                                          ]).astype('float64')
    # m_void_prev         = m_void
    # m_particle_tot_prev = m_particle_tot
    # temp_celsius_prev   = temp_celsius
    # # water_activity_prev = water_activity
    # gas_density_prev    = gas_density_curr
...
def plot_tam():
    ...
    # plt.savefig('system_w_derivative6.pdf')
    # plt.savefig('system_w_derivative7.pdf')   # 16% am
    # plt.savefig('system_w_derivative8.pdf')     # 1% am
...
temp_sur_initial_vector     = np.zeros(n_space_steps) + temp_initial_celsius
# temp_sur_initial_vector     = np.zeros(n_space_steps) + enthalpy_sur_air_initial   # J/m3
excess_water_prev = 0

#TODO: change initials
initials = np.concatenate([m_tot_conc_vector, initial_amorphicity_vector, energy_initial_vector,
                           water_activity_initial_vector, m_sur_initial_vector])    #, temp_sur_initial_vector
# initials = np.concatenate([m_tot_conc_vector, initial_amorphicity_vector, energy_initial_vector,
#                            water_activity_initial_vector, m_sur_initial_vector, energy_sur_vapor_initial_vector])      # kg/m3, kg/kg
...
# water_activity_prev = np.zeros(n_space_steps) + water_activity_void_initial
# # temp_celsius_sur_prev = np.zeros(n_space_steps) + temp_initial_celsius
# gas_density_prev = gas_density_initial
...
for t in range(1, 1+resolution):
    ...
    m_void_all[t, :] = compute_H_from_water_activity_temp(water_activity_all[t - 1, :])
    ...
    water_W_all[t-1, :]           = porosity_powder * m_void_all[t-1, :] * density_gas_all[t-1, :] + (1 - porosity_powder) * m_powder_total_all[t-1, :] * density_particle
    excess_water_curr[t-1, :]     = total_water_all[t-1, :] - water_W_all[t-1, :]
    ...
    density_gas_all[t-1, :]           = compute_density_air(m_void_all[t-1, :])
