...
def compute_temperature_gas(temperature, constant, dt, conductivity_gas, laplacian, density, alpha, moisture,
                                 heat_capacity_vapor, relative_humidity, N, heat_transfer_coefficient, specific_surface,
                                 temperature_gas, heat_capacity_wet_gas, velocity, temp_gradient):
    conduction = conductivity_gas * laplacian / density
    heat_of_sorption = constant * (relative_humidity - compute_equilibrium_moisture(alpha, moisture, N)) * \
                       heat_capacity_vapor * (temperature_gas - temperature)
    heat_transfer = -heat_transfer_coefficient * specific_surface * (temperature_gas - temperature)

    change_temperature = (heat_of_sorption + heat_transfer) / heat_capacity_wet_gas - velocity * temp_gradient
    temperature += change_temperature * dt
    return temperature

def compute_moisture_gas(temperature, constant, dt, conductivity, laplacian, density, alpha, moisture,
                                 relative_humidity, N, heat_of_vaporization, heat_transfer_coefficient, specific_surface,
                                 temperature_gas, heat_capacity):
    pass
...
def mass_transfer_coefficient(moisture_diffusivity, gas_viscosity, column_diameter, porosity_powder, gas_density, 
    ...
    # reynolds_number = superficial_mass_velocity / particle_surface_area * gas_viscosity
    reynolds_number = superficial_mass_velocity/(particle_surface_area * gas_viscosity) #Re # TODO: CHECK!!

    denominator = gas_viscosity/(gas_density * moisture_diffusivity)
    j_m = 0.61 * reynolds_number ** -0.41

    k_gp = j_m * (molar_concentration * Mw) * relative_velocity_gp/(denominator ** (2/3))
